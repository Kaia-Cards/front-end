...
interface DappPortalSDKType {
  getWalletProvider: () => any;
}

interface KaiaWalletSdkState {
...
export const initializeKaiaWalletSdk = async () => {
  try {
    ...
    const sdk = await DappPortalSDK.default.init({
      clientId: import.meta.env.VITE_KAIA_CLIENT_ID as string,
